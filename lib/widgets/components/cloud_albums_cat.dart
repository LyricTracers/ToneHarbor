...
class CloudAlbumsCat<T extends ExtraProvider<CloudAlbumListData>>
    extends HookConsumerWidget {
  const CloudAlbumsCat({
    super.key,
    this.visibleRows = -1,
      ...
      void onScroll() {
        ...
        if (currentScroll >= maxScroll * 0.8) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            ...
            if (currentAlbums.length < currentTotal) {
  ...
  Widget _buildGrid(
    ...
    final albums = albumList.albums ?? [];
    final total = albumList.total ?? 0;

    final hasMore = visibleRows == -1 && albums.length < total;
    ...
    return Padding(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          List<CloudMusicAlbumData> displayItems;
          if (visibleRows != -1) {
            final maxItems = crossAxisCount * visibleRows;
            displayItems = albums.length > maxItems
                ? albums.sublist(0, maxItems)
                : albums;
          } else {
            displayItems = albums;
          }
...
class CloudMusicAlbumItem extends StatelessWidget {
  const CloudMusicAlbumItem({
    super.key,
    required this.album,
...
class CloudMusicAlbumItemShimmer extends StatelessWidget {
  const CloudMusicAlbumItemShimmer({super.key, required this.colorScheme});
